...
if (( $(python -c "print(int(${GRAIN_SIZE} > 0))") )); then
...
# Output filename
NI_INT=$(python -c "print(int(${NI_PERCENT}))")
...
echo "Size:          ${SIZE} nm"
echo "Shape:         ${SHAPE}"
echo "Composition:   Ni${NI_INT}Ti$(python -c "print(100-${NI_INT})")"
echo "Structure:     ${TYPE}"
echo "Seed:          ${SEED}"
echo "=========================================="

# Build command
CMD="python \"${PROJECT_ROOT}/src/ase_np_gen.py\" \
    --diameter ${SIZE} \
    --shape ${SHAPE} \
...
if (( $(python -c "print(int(${GRAIN_SIZE:-0} > 0))") )); then
...
if [[ -f "${OUTPUT}" ]]; then
    ...
    echo "File sizes:"
    echo "  LAMMPS: $(python -c "print(f'{${LAMMPS_SIZE}/1024:.1f} KB')" 2>/dev/null || echo "${LAMMPS_SIZE} bytes")"
    echo "  XYZ:    $(python -c "print(f'{${XYZ_SIZE}/1024:.1f} KB')" 2>/dev/null || echo "${XYZ_SIZE} bytes")"
    echo "=========================================="
